@extends('layouts.app')
@section('content')
<div class="row">
  <div class="col-lg-12 margin-tb">
    <div class="pull-left">
      <h2><a href="{{ route('posts.index') }}"><button class="btn btn-primary">Back</button></a> Show Post </h2>
    </div>
  </div>
</div>
<div class="adduser" style="margin-top: 20px">
  <div class="cardHeader" style="text-align: center;">
    {{ $post->post_name }}
  </div>
  <b style="margin-left: 10px;">Posted at {{ $post->created_at->diffForHumans() }}</b>

  <div class="register-form">
    <img src="{{ asset('post_img/' . $post->post_img) }}" class="post_img" style="width: 500px;margin-top:20px; height:auto;" />
    <div style="box-shadow:0px 1px 1px gray; padding:20px; margin-bottom:20px; border-radius:5px;">
      <p style="margin:0;padding:0;text-indent:50px">{{ $post->detail }}</p>
    </div>
  </div>
  @if ($message = Session::get('success'))
  <div class="alert alert-success">
    <p>{{ $message }}</p>
  </div>
  @endif
  @if ($message = Session::get('error'))
  <div class="alert alert-danger">
    <p>{{ $message }}</p>
  </div>
  @endif
  <ul class="list-group mb-2">
    <li class="list-group-item active">
      <b>Comments ({{ count($post->comments) }})</b>
    </li>
    @foreach($post->comments as $comment)
    <li class="list-group-item">
      <a href="{{ route('commentDelete',$comment->id) }}" style="float:right;text-decoration:none">&#x2715</a>
      {{ $comment->content }}
      <div class="small mt-2">
        written by <b>{{ $comment->user->name }}</b>
        <b style="float: right;">{{ $comment->created_at->diffForHumans() }}</b>
      </div>
    </li>
    @endforeach
  </ul>
  @auth
  <form action="{{ url('/comments/add') }}" method="post">
    @csrf
    <input type="hidden" name="post_id" value="{{ $post->id }}">
    <textarea name="content" class="form-control mb-2" placeholder="New Comment..."></textarea>
    <input type="submit" value="Add Comment" class="btn btn-secondary">
  </form>
  @endauth

</div>


@endsection